...
// Define interface for the Alive document
interface IAlive extends Document {
    id: string;
    alive_text: string;
    alive_get: string;
    alive_url: string;
    ...
    autoreaction: string;
    permit: boolean;
    permit_values: string;
    chatbot: string;
    bgm: boolean;
    bgmarray: Record<string, unknown>;
...
// Define the Alive schema
const AliveSchema: Schema = new Schema<IAlive>({
    id: { type: String, unique: true, required: true, default: "Suhail_Md" },
    alive_text: {
        type: String,
        default: `*ι αм σηℓιηє нσω ¢αη ι нєℓρ уσυ*\n\n_ι αм ᴍυℓтι ԃєνιᴄє ωнαтѕαρρ вσт_\n_Cʀєαtєd вყ : ѕυнαιℓ tєᴄн info_\n_If any query : wa.me/94775704025_\n\n\n*_Update Alive Message by adding text with Alive_*\n*Eg: .alive Your New_Alive_Message_*`,
    },
    alive_get: { type: String, default: "you didn't set alive message yet\nType [.alive info] to get alive message information" },
    alive_url: { type: String, default: "" },
    alive_image: { type: Boolean, default: "./lib/assets/suhail.jpg" },
    alive_audio: { type: Boolean, default: "./lib/assets/Disinduphonk.mp3" },
    audio_url: { type: String, default: "./lib/assets/Disinduphonk.mp3" },
    alive_video: { type: Boolean, default: false },
    antiviewonce: { type: String, default: "true" },
    antidelete: { type: String, default: "true" },
    autobio: { type: String, default: "true" },
    levelup: { type: String, default: "true" },
    anticall: { type: String, default: "false" },
    autoreaction: { type: String, default: "yes bruh?" },
    permit: { type: Boolean, default: false },
    permit_values: { type: String, default: "all" },
    chatbot: { type: String, default: "true" },
    bgm: { type: Boolean, default: false },
    bgmarray: { type: Object, default: {} },
    plugins: { type: Object, default: {} },
    notes: { type: Object, default: {} },
    mention: { type: Object, default: {} },
    filter: { type: Object, default: { suhail_: "yes bruh?" } },
    afk: { type: Object, default: {} },
    ...options,
});

// Create the Alive model
const Alive: Model<IAlive> = mongoose.model<IAlive>("Alive", AliveSchema);

// Function to send an alive message with audio, text, and optional image
const sendAliveMessage = async (id: string): Promise<{
    audio?: string;
    message: string;
    image?: string;
}> => {
    try {
        // Fetch the alive document
        const aliveDoc = await Alive.findOne({ id });

        if (!aliveDoc) {
            throw new Error("Alive message configuration not found for this ID.");
        }

        // Prepare the alive message
        const aliveMessage = aliveDoc.alive_text;
        let response: { audio?: string; message: string; image?: string } = { message: aliveMessage };

        // Check if an audio file is configured
        if (aliveDoc.alive_audio && aliveDoc.audio_url) {
            response.audio = aliveDoc.audio_url;
        }

        // Check if an image is configured
        if (aliveDoc.alive_image && aliveDoc.alive_url) {
            response.image = aliveDoc.alive_url;
        }
...
// Export the model and sendAliveMessage function
export { Alive, sendAliveMessage };
